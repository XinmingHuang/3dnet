...
from tasks.semantic.modules.3Dnet import *
from tasks.semantic.modules.3DnetAdf import *
...
def save_checkpoint(to_save, logdir, suffix=""):
    # Save the weights
    #torch.save(to_save, logdir +  "/Bisenet" + suffix, _use_new_zipfile_serialization=False)
    torch.save(to_save, logdir + "/Bisenet" + suffix+'.pth')
    #if not os.path.isdir('checckpoint'):
       # os.mkdir('checkpoint')
   # torch.save(to_save,logdir +'ckpt_'  + suffix +'.pth')
    #torch.save(to_save,'./checkpoint/ckpt_'+ '.pth')
...
class Trainer():
    def __init__(self, ARCH, DATA, datadir, logdir, path=None,uncertainty=False):
        ...
        # get the data
        parserModule = importlib.machinery.SourceFileLoader("parserModule",
                                       booger.TRAIN_PATH + '/tasks/semantic/dataset/' +
                                       self.DATA["name"] + '/parser.py').load_module()
        ...
        with torch.no_grad():
            if not self.uncertainty:
                self.model = 3Dnet(self.parser.get_n_classes())
            else:
                self.model = 3DnetUncertainty(self.parser.get_n_classes())
    ...
    def train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10,
        ...
        for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _) in enumerate(train_loader):
            ...
            if self.uncertainty:
                output = model(in_vol)
                output_mean, output_var = adf.Sofctmax(dim=1, keep_variance_fn=keep_variance_fn)(*output)
